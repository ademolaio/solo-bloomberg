...
def main(asset_class: str = "equity", is_active: int = 1):
    ...
    # Annual + Quarterly
    backfill_all_symbols(client, table_fq=TABLE, statement="income_statement", period="annual", symbols=symbols, source=SOURCE)
    backfill_all_symbols(client, table_fq=TABLE, statement="income_statement", period="quarterly", symbols=symbols, source=SOURCE)

    print("[ok] income_statement: DDL + backfill completed")
