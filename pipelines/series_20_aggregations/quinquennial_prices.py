# pipelines/series_20_aggregations/quinquennial.py
...
DDL = f"""
DROP TABLE IF EXISTS {DB}.quinquennial_prices;

CREATE TABLE {DB}.quinquennial_prices
(
  instrument_id UUID,
  source LowCardinality(String),

  quinquennial_start Date,
  quinquennial_end   Date,
...
PARTITION BY toYYYYMM(quinquennial_start)
ORDER BY (instrument_id, source, quinquennial_start);

DROP VIEW IF EXISTS {DB}.mv_quinquennial_prices;

CREATE MATERIALIZED VIEW {DB}.mv_quinquennial_prices
TO {DB}.quinquennial_prices
AS
SELECT
  instrument_id,
  source,
  ...
  toDate(
    concat(
      toString(intDiv(toYear(date), 5) * 5),
      '-01-01'
    )
  ) AS quinquennial_start,

  max(date) AS quinquennial_end,
...
GROUP BY instrument_id, source, quinquennial_start;

CREATE OR REPLACE VIEW {DB}.v_quinquennial_prices AS
SELECT
  instrument_id,
  source,
  quinquennial_start,
...
FROM {DB}.quinquennial_prices
GROUP BY instrument_id, source, quinquennial_start, quinquennial_end;
"""

BACKFILL_TEMPLATE = f"""
INSERT INTO {DB}.quinquennial_prices
SELECT
  instrument_id,
  source,
...
def main(min_year: int = 1970, max_year: int = 2026):
    ...
    # Year-by-year backfill to avoid "max_partitions_per_insert_block" issues
    backfill_by_ranges(
        client,
    ...
    print("[ok] quinquennial: DDL + MV + backfill completed")
