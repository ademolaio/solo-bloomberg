...
DDL = f"""
DROP TABLE IF EXISTS {DB}.monthly_prices;

CREATE TABLE {DB}.monthly_prices
(
  instrument_id UUID,
...
DROP VIEW IF EXISTS {DB}.mv_monthly_prices;

CREATE MATERIALIZED VIEW {DB}.mv_monthly_prices
TO {DB}.monthly_prices
AS
SELECT
  instrument_id,
  source,
...
GROUP BY instrument_id, source, month_start;

CREATE OR REPLACE VIEW {DB}.v_monthly_prices AS
SELECT
  instrument_id,
  source,
  month_start,
...
FROM {DB}.monthly_prices
GROUP BY instrument_id, source, month_start, month_end;
"""


BACKFILL_TEMPLATE = f"""
INSERT INTO {DB}.monthly_prices
SELECT
  instrument_id,
  source,
  toStartOfMonth(date) AS month_start,
...
def main(min_year: int = 1970, max_year: int = 2026):
    ...
    print("[ok] monthly: DDL + MV + backfill completed")
